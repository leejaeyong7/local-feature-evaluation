...
class D2Net(nn.Module):
    ...
    def forward(self, image):
        ...
        all_keypoints[2] = h_init - all_keypoints[2]
        return all_keypoints.t()[:, :2].detach(), all_descriptors.t().detach()
...
def preprocess_image(image):
    '''
    VGG16 preprocessing for caffe models
    '''
    image = image[:, [2, 1, 0], :, :]
    mean = torch.tensor([103.939, 116.779, 123.68])
    return (image * 255 - mean.view(1, 3, 1, 1))
