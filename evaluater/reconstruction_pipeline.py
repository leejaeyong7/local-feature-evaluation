...
def import_matches(dataset_path, colmap_path):

    connection = sqlite3.connect(os.path.join(dataset_path, "database.db"))
    ...
    for image_name, image_id in images.items():
        ...
        descriptor_path = os.path.join(dataset_path, "descriptors",
                                     image_name + ".bin")
    ...
    image_pairs = []
    image_pair_ids = set()
    for match_path in glob.glob(os.path.join(dataset_path,
                                             "matches/*---*.bin")):
    ...
    with open(os.path.join(dataset_path, "image-pairs.txt"), "w") as fid:
    ...
    subprocess.call([os.path.join(colmap_path, "colmap"),
                     "matches_importer",
                     "--database_path",
                     os.path.join(dataset_path, "database.db"),
                     "--match_list_path",
                     os.path.join(dataset_path, "image-pairs.txt"),
                     "--match_type", "pairs"])
...
def reconstruct(dataset_path, colmap_path):
...
def reconstruction_pipeline(extractor_type, dataset_dir, colmap_dir, results_dir):
    method_name = extractor_type
    matching_stats = import_matches(dataset_dir, colmap_dir)
    reconstruction_stats = reconstruct(dataset_dir, colmap_dir)

    dataset_name = os.path.basename(dataset_dir)
    method_result_dir = os.path.join(results_dir, method_name)
    ...
    result_file = os.path.join(method_result_dir, dataset_name + '.pickle')
